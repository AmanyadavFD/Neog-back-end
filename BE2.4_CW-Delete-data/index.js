...
async function deleteMovie(movieId) {
  try {
    const movieDelete = await Movie.findByIdAndDelete(movieId);
...
// deleteMovie("67954a41cc76dd20c96a98b4");

async function deleteMovieByDB(movieTitle) {
  try {
    const deleteSingleMovie = await Movie.findOneAndDelete({
      title: movieTitle,
    });

    console.log(deleteSingleMovie);
  } catch (error) {
    console.log("Error in Deleting Movie", error);
  }
}

deleteMovieByDB("Dilwale Dulhania Le Jayenge");
